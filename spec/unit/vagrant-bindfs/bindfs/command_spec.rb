...
describe VagrantBindfs::Bindfs::Command do
  it 'should build bindfs command as expected' do
    ...
    expect(command.to_s).to eq('bindfs --force-user=vagrant --chown-deny -o debug,allow_others -r /etc /etc-binded')
  end
  ...
  it 'should build bindfs command according to bindfs version' do
    ...
    folder.to_version!('1.13.3')
    command = described_class.new(folder)
    expect(command.to_s).to eq('bindfs --force-user=vagrant --force-group=vagrant --perms=u=rwX:g=rD:o=rD /etc /etc-binded')

    folder.to_version!('1.11')
    command = described_class.new(folder)
    expect(command.to_s).to eq('bindfs --user=vagrant --group=vagrant --perms=u=rwX:g=rD:o=rD /etc /etc-binded')
  end
end
